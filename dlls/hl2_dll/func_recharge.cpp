...
void CRecharge::Spawn()
{
	...
	UpdateJuice( static_cast<int>(MaxJuice()) );
...
void CNewRecharge::Off(void)
{
	...
	if ( m_iReactivate == 0 )
	{
		if ((!m_iJuice) && g_pGameRules->FlHEVChargerRechargeTime() > 0 )
		{
			if ( HasSpawnFlags( SF_CITADEL_RECHARGER ) )
			{
				m_iReactivate = static_cast<int>(g_pGameRules->FlHEVChargerRechargeTime() * 2);
			}
			else
			{
				m_iReactivate = static_cast<int>(g_pGameRules->FlHEVChargerRechargeTime());
			}
			SetNextThink( gpGlobals->curtime + m_iReactivate );
