...
void CPropAirboat::ExitVehicle( int nRole )
{
	...
	ep.m_nPitch = static_cast<int>(controller.SoundGetPitch( m_pEngineSound ));
...
void CPropAirboat::CreateDangerSounds( void )
{
	...
	if ( fabs(speed) > 120 )
	{
		...
		CSoundEnt::InsertSound( SOUND_DANGER, vecSpot, static_cast<int>(radius), soundDuration, this, SOUNDENT_CHANNEL_REPEATED_DANGER );
		CSoundEnt::InsertSound( SOUND_PHYSICS_DANGER, vecSpot, static_cast<int>(radius), soundDuration, this, SOUNDENT_CHANNEL_REPEATED_PHYSICS_DANGER );
