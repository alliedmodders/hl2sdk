...
void CBaseProp::DrawDebugGeometryOverlays( void )
{
	...
	if ( m_debugOverlays & OVERLAY_PROP_DEBUG )  
	{
		...
		else
		{
			...
			flG = clamp( flG, 0, 255 );
			NDebugOverlay::EntityBounds(this, 0, static_cast<int>(flG), 0, 0, 0 );
		}
	}
}
...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if ( m_explodeDamage > 0 || m_explodeRadius > 0 )
	{
		if( HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE ) )
		{
			ExplosionCreate( WorldSpaceCenter(), angles, pAttacker, static_cast<int>(m_explodeDamage), static_cast<int>(m_explodeRadius), 
	...
	if( HasInteraction( PROPINTER_PHYSGUN_BREAK_EXPLODE ) )
	{
		if ( bExploded == false )
		{
			ExplosionCreate( origin, angles, pAttacker, 1, static_cast<int>(m_explodeRadius), 
...
void CPropDoorRotating::BeginOpening(CBaseEntity *pOpenAwayFrom)
{
	...
	{
		CSoundEnt::InsertSound( SOUND_MOVE_AWAY, volumeCenter, static_cast<int>(volumeRadius), 0.5f, pOpenAwayFrom );
	}
