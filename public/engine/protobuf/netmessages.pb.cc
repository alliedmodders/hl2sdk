...
#define ZR_(first, last) do {                              \
  ...
  if (_has_bits_[0 / 32] & 255) {
    ZR_(client_, xuid_);
    ZR_(audible_mask_, section_number_);
    if (has_voice_data()) {
      if (voice_data_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
        voice_data_->clear();
      }
  ...
  ZR_(uncompressed_sample_offset_, tick_);
